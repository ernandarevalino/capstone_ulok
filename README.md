...
This application is a comprehensive web-based decision support system designed to manage and assess location proposals (_Usulan Lokasi_ or **ULOK**) for **PT. Midi Utama Indonesia Tbk (Alfamidi)**. It bridges the gap between digitalizing legal documentation and calculating location feasibility for store expansions.
...
> [!NOTE]
> _Legacy Mapping Note_: In earlier system designs, queues were referred to as **Kelompok 1** (Draft/Low progress), **Kelompok 2** (Active Verification), **Kelompok 3** (Revision), and **Kelompok 4** (Approved/100% complete). The current codebase implements this mapping dynamically across the four groups above, keeping Drafts excluded from Assessor lists to optimize focus.
...
Measures the percentage of verified required documents against the checklist dynamically determined by the submitter's legal entity type (_Jenis Badan Hukum_):
...
- **Final Score $\ge 0.75$**: Primary Recommendation (_Rekomendasi Utama_). The proposal has robust document completion, low cost, or rapid approval times.
...
- **Integration Clients & Architecture**:
  - `@supabase/supabase-js` v2.105.4 (`lib/supabaseClient.ts` for client-side queries)
  - `@supabase/ssr` v0.10.3 (`utils/supabase/server.ts` for server-side queries/SSR)
  - **Next.js Server Actions**: Exclusively mapped in `actions/` folder for DB mutations (INSERT, UPDATE, DELETE).
- **Prisma ORM Clarification**: PRISMA is the acronym for the application name (**Prioritizing Location**). This project **DOES NOT** use Prisma ORM. It strictly interacts with Supabase using the `@supabase/supabase-js` client SDK.
- **1:1 Auto-Initialized Sub-tables**: Every row in `ulok_submissions` automatically initializes 1:1 placeholder rows in the following 5 sub-tables:
  1. `ulok_pemilik` (Owner Identities & identification fields)
  2. `ulok_sertifikat` (Land Certificate details and process records)
  3. `ulok_legal` (AJB details and subdistrict letters)
  4. `ulok_jaminan` (Bank Guarantee descriptions)
  5. `metode_saw` (SAW metrics, scores, and auto-generated audit notes)
...
│   └── page.tsx           # Welcome landing page (PRISMA Landing)
