...
import { Trophy, Medal, AlertCircle, Calendar, Sparkles, ChevronDown, ChevronUp, MapPin, Award, UserCheck } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 p-6 flex items-center justify-center">
        <div className="bg-white p-6 rounded-2xl border shadow-lg max-w-md text-center space-y-4">
          ...
          <button onClick={loadData} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-4 py-2 rounded-xl text-xs transition">
            Coba Lagi
          </button>
        </div>
      </div>
        ...
        <div className="bg-gradient-to-r from-emerald-950 via-slate-900 to-emerald-950 text-white p-8 rounded-3xl shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden border border-emerald-900/55">
          ...
          <div className="space-y-2 z-10">
            <div className="flex items-center gap-2">
              <span className="bg-amber-500 text-emerald-950 font-bold px-2.5 py-1 rounded-full text-[10px] uppercase tracking-wider flex items-center gap-1">
                <Sparkles className="w-3 h-3 fill-current" /> Leaderboard Nasional Cabang
              </span>
            </div>
            <h1 className="text-2xl font-black tracking-tight">Peringkat Kompetitif Usulan Lokasi Nasional</h1>
            <p className="text-xs text-emerald-200/80 max-w-2xl leading-relaxed">
              Bandingkan kualitas usulan ruko Anda dengan kantor cabang lainnya secara nasional! Penilaian menggunakan kriteria C1 (Legalitas), C2 (Kecepatan Pengurusan Dokumen Wajib), dan C3 (Efisiensi Biaya Sewa). Usulan cabang Anda akan disorot khusus dengan warna hijau menyala.
            </p>
          </div>
          <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/10 text-center z-10 w-full md:w-auto">
            <p className="text-[10px] text-emerald-200 uppercase font-bold tracking-wider">Total Usulan Ternilai</p>
        ...
        <div className="space-y-4">
          {leaderboard.length === 0 ? (
            <div className="bg-white rounded-2xl border p-12 text-center shadow-sm text-gray-500">
              ...
              <p className="font-bold text-sm">Belum ada usulan lokasi yang diajukan secara nasional.</p>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 w-full md:w-auto">
                      ...
                      <div className="space-y-1 min-w-0">
                        <h3 className="font-black text-base text-gray-900 truncate flex flex-wrap items-center gap-2">
                          {item.nama_lokasi}
                          
                          {isCurrentUserBranch && (
                            <span className="bg-emerald-100 text-emerald-800 border border-emerald-300 font-bold px-2 py-0.5 rounded text-[9px] uppercase tracking-wider flex items-center gap-1">
                              <UserCheck className="w-3 h-3" /> Usulan Anda
                            </span>
                          )}
                        ...
                        <div className="flex flex-wrap items-center gap-y-1 gap-x-3 text-xs text-gray-500 font-medium">
                          <span className={`flex items-center gap-1 font-bold ${isCurrentUserBranch ? 'text-emerald-700' : 'text-gray-700'}`}>
                            <MapPin className="w-3.5 h-3.5" /> Cabang {branchName}
                          </span>
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto border-t md:border-none pt-3 md:pt-0">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          ...
                          <p className={`text-2xl font-black leading-tight ${isCurrentUserBranch ? 'text-emerald-700' : 'text-slate-800'}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-white p-3.5 rounded-xl border">
                          ...
                          <span className="text-xs font-bold text-gray-700 mt-1 block">
                            Skor: <strong className="text-slate-800 text-sm font-black">{item.c1_score || 1}</strong> dari 5
                          </span>
                        ...
                        <div className="bg-white p-3.5 rounded-xl border">
                          ...
                          <span className="text-xs font-bold text-gray-700 mt-1 block">
                            Skor: <strong className="text-slate-800 text-sm font-black">{item.c3_score || 1}</strong> dari 5
                          </span>
                          <p className="text-[10px] text-gray-400 mt-1">
