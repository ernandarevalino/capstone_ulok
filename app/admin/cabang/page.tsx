      ...
      try {
        ...
        if (submissionsRes && submissionsRes.success && submissionsRes.data) {
          ...
          const uncalculated = submissionsRes.data.filter(
            (s: any) =>
              s.status !== 'Draft' &&
              (s.final_score === 0 || s.final_score === null)
          );

          if (uncalculated.length > 0) {
            Promise.all(
              uncalculated.map((s: any) => calculateULOKSAW(s.id))
            )
              .then(() => {
              ...
              .catch((err) =>
                console.error(
                  "Gagal melakukan kalkulasi background SAW:",
                  err
                )
              );
          }
        ...
        console.error(
          "Gagal menginisialisasi dashboard admin cabang:",
          err
        );
      } finally {
        setLoading(false);
    ...
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto md:p-6 lg:p-8 text-gray-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div className="group bg-white dark:bg-slate-900 p-3.5 sm:p-5 rounded-xl sm:rounded-2xl border-t-4 border-t-[#142B4D] shadow-sm flex items-center justify-between gap-2 transition-all duration-300 sm:hover:-translate-y-1 hover:shadow-md active:scale-[0.98]">
          ...
          <div className="space-y-0.5 sm:space-y-1 min-w-0">
            ...
            <p className="text-gray-400 dark:text-slate-500 text-[10px] sm:text-xs font-bold uppercase tracking-wider truncate">
              Total Pengajuan
            </p>
          ...
          <div className="bg-[#142B4D]/10 dark:bg-slate-800 p-2 sm:p-3 rounded-lg sm:rounded-xl shrink-0 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6">
            <FileText className="w-4.5 h-4.5 sm:w-6 sm:h-6 text-[#142B4D] dark:text-blue-400" />
          </div>
        ...
        <div className="group bg-white dark:bg-slate-900 p-3.5 sm:p-5 rounded-xl sm:rounded-2xl border-t-4 border-t-slate-400 shadow-sm flex items-center justify-between gap-2 transition-all duration-300 sm:hover:-translate-y-1 hover:shadow-md active:scale-[0.98]">
          ...
          <div className="space-y-0.5 sm:space-y-1 min-w-0">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider truncate">
              Status Draft
            </p>

            <h3 className="text-xl sm:text-3xl font-black text-slate-700 dark:text-slate-200 tracking-tight">
              {draftCount}
            </h3>

            <p className="hidden sm:block text-[11px] text-gray-400 dark:text-slate-400 font-medium">
              Belum diajukan ke pusat
            </p>

          </div>

          <div className="bg-slate-50 dark:bg-slate-800/60 p-2 sm:p-3 rounded-lg sm:rounded-xl shrink-0 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6">
            <FileEdit className="w-4.5 h-4.5 sm:w-6 sm:h-6 text-slate-600 dark:text-slate-400" />
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl p-4 md:p-6 shadow-sm transition-all duration-300 hover:shadow-md">
        ...
        <div className="border-b border-gray-100 dark:border-slate-800/80 pb-3 sm:pb-4 mb-3 sm:mb-4">
          ...
          <p className="text-[11px] sm:text-xs text-gray-400 dark:text-slate-500 mt-0.5">
            Visualisasi penyebaran status usulan lokasi saat ini
          </p>

        </div>

        <div className="flex flex-col lg:flex-row items-center justify-around gap-4 md:gap-6 w-full">
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-2">
            ...
            <h4 className="text-xs font-extrabold uppercase text-gray-400 dark:text-slate-500 tracking-wider mb-1 sm:mb-2">
              Legenda Status
            </h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl shadow-sm overflow-hidden flex flex-col transition-all duration-300 hover:shadow-md">
          ...
          <div className="p-3 sm:p-4 flex-1 divide-y divide-gray-100 dark:divide-slate-800 overflow-y-auto max-h-[300px] sm:max-h-[350px] scrollbar-thin">
            ...
            {notifications.length === 0 ? (
              ...
              <div className="text-center py-12 text-xs text-gray-400 dark:text-slate-500 italic">
              ...
              notifications.map((notif, idx) => (
                ...
                <div
                  key={notif.id || idx}
                  className="py-2.5 sm:py-3 first:pt-0 last:pb-0 flex gap-2.5 sm:gap-3 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition px-2 rounded-xl group"
                >
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl shadow-sm overflow-hidden flex flex-col transition-all duration-300 hover:shadow-md">
          ...
          <div className="overflow-x-auto flex-1 scrollbar-thin -webkit-overflow-scrolling-touch">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800/50 text-gray-700 dark:text-slate-300">
                ...
                {topScores.length === 0 ? (
                  ...
                  <tr>
                    ...
                    <td
                      colSpan={4}
                      className="text-center p-12 text-xs text-gray-400 dark:text-slate-500 italic"
                    >
                      Belum ada lokasi yang dinilai.
                    </td>
                  ...
                  topScores.map((row, idx) => {
                    ...
                    if (row.status === 'Approved') {
                      ...
                      statusColor =
                        "bg-[#FE9A00]/10 text-[#FE9A00] border border-[#FE9A00]/20 dark:bg-[#FE9A00]/20";
                    } else if (row.status === 'Revisi') {
                      statusColor =
                        "bg-[#D11A22]/10 text-[#D11A22] border border-[#D11A22]/20 dark:bg-[#D11A22]/20";
                    }
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-2.5 sm:p-3 pl-4 sm:pl-5 font-bold text-gray-900 dark:text-slate-100 group-hover:text-[#142B4D] dark:group-hover:text-blue-400 transition-colors">
                          {row.nama_lokasi}
                        </td>

                        <td className="p-2.5 sm:p-3 text-gray-500 dark:text-slate-400 font-medium">
                          {row.jenis_badan_hukum}
                        </td>

                        <td className="p-2.5 sm:p-3 text-center">

                          <span
                            className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold ${statusColor}`}
                          >
                            {row.status === 'Draft'
                              ? 'Draf'
                              : row.status}
