  ...
  const triggerMidilocSync = useCallback(async (targetNomor?: string) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        // If legal type changed from Badan Hukum to Perorangan, redirect to perorangan form
        const isPerorangan = ['Perorangan', 'Waris', 'Hibah', 'Kuasa'].includes(synced.jenis_badan_hukum)
        if (isPerorangan) {
          router.push(`/admin/cabang/usulan-lokasi/form/perorangan?id=${ulokId}${fromSource ? `&from=${fromSource}` : ''}`)
        }
      } else {
