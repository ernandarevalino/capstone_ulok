...
import { Check, Loader2, AlertCircle, Download, History, CheckCircle2 } from 'lucide-react'
                ...
                {openAccordions[docType] && (
                  <div className="mt-1.5 space-y-1.5 pl-0.5 max-h-36 overflow-y-auto w-full">
                    {historyFiles.map((file) => (
                      ...
                      >
                        <div className="flex flex-col min-w-0">
                          <span className="text-[8px] font-extrabold text-gray-600 dark:text-gray-400 flex items-center gap-1">
                            <span className="bg-gray-200 dark:bg-gray-800 text-gray-500 dark:text-gray-400 px-1 py-0.2 rounded text-[7px] font-bold">
                              v{file.version || 1}
                            </span>
                            Versi Lama
                          </span>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-6 text-gray-800 dark:text-gray-200 transition-colors duration-300">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-white dark:bg-[#111827] p-4 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
          ...
          <button 
            type="button" 
            onClick={() => router.push(`/admin/assessor/penilaian/ulok-perorangan/detail-penilaian/section2?id=${ulokId}${fromSource ? `&from=${fromSource}` : ''}`)} 
