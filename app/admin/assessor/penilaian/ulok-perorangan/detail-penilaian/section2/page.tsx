...
export default function Section2PeroranganAssessorPage() {
  ...
  const handleReplyDocument = (docName: string, fileName?: string) => {
    ...
    router.push(`/admin/assessor/penilaian/ulok-perorangan?id=${ulokId}&prefill=${encodeURIComponent(prefix)}${fromSource ? `&from=${fromSource}` : ''}`)
  }
  ...
  const handleSelesaiPenilaian = () => {
    ...
    setTimeout(() => {
      setShowSuccessModal(false)
      router.push(`/admin/assessor/penilaian/ulok-perorangan?id=${ulokId}${fromSource ? `&from=${fromSource}` : ''}`)
    }, 1500)
  }
                      ...
                      >
                        <div className="flex flex-col min-w-0">
                          <span className="text-[8px] font-extrabold text-gray-655 dark:text-gray-400 flex items-center gap-1">
                            <span className="bg-gray-200 dark:bg-gray-800 text-gray-550 dark:text-gray-405 px-1 py-0.2 rounded text-[7px] font-bold">
                              v{file.version || 1}
                            </span>
                            Versi Lama
                          </span>
                        ...
                        <div className="flex gap-1 items-center shrink-0">
                          <a 
                            ...
                            target="_blank" 
                            rel="noopener noreferrer" 
                            className="p-1 rounded bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-xs text-gray-655 dark:text-gray-450 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-300 transition-all flex items-center justify-center"
                            title="View File"
                          >
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-6 text-gray-800 dark:text-gray-200 transition-colors duration-300">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <nav className="flex items-center gap-1 text-xs font-bold text-gray-500 dark:text-gray-400 select-none mb-10 mt-2 uppercase tracking-wider">
          ...
          <span 
            onClick={() => router.push(`/admin/assessor/penilaian/ulok-perorangan?id=${ulokId}${fromSource ? `&from=${fromSource}` : ''}`)} 
